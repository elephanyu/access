...
class OracleReaderTimeThread(BaseThread):
    ...
    def new_connection(self):
        try:
            connection = cx_Oracle.connect("%s/%s@%s:%s/%s" % (
                    self.proc_setting['setting']['user'],
                    self.proc_setting['setting']['password'],
                    self.proc_setting['setting']['ip'],
                    self.proc_setting['setting']['port'],
                    self.proc_setting['setting']['dbname']))
            return connection
        except Exception:
    ...
    def process(self):
        ...
        if rows is not None and len(rows) != 0:
            last_data = data_mapping(rows[-1])
            # 此次查询数据的所有过车时间都和上一次查询数据的最大过车时间相同
            if last_data['snapshotTime'] == to_mstimestamp(self.current_time):
                self.current_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.mktime(time.strptime(self.current_time, '%Y-%m-%d %H:%M:%S'))+1))
